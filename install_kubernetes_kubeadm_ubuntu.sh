...
# - Installs Docker CE and CLI (for troubleshooting and compatibility)
...
# -------- Add Docker apt repository and install Docker CE/CLI --------
log "Adding Docker apt repository and keyring..."
...
log "Installing Docker CE and CLI..."
apt-get install -y docker-ce docker-ce-cli containerd.io
systemctl enable --now docker

# -------- Install and configure containerd --------
install_containerd() {
  log "Ensuring containerd is installed..."
  ...
  sed -ri 's|^disabled_plugins = \["cri"\]|# disabled_plugins = ["cri"]|' /etc/containerd/config.toml
...
if ! cmd_exists containerd; then
  ...
  if [[ -f /etc/containerd/config.toml ]]; then
    ...
    sed -ri 's|^disabled_plugins = \["cri"\]|# disabled_plugins = ["cri"]|' /etc/containerd/config.toml || true
  else
    mkdir -p /etc/containerd
    containerd config default >/etc/containerd/config.toml || true
    sed -ri 's|^(\s*SystemdCgroup\s*=\s*)false|\1true|' /etc/containerd/config.toml || true
...
# -------- Initialize control plane (if requested) --------
if $DO_CONTROL_PLANE; then
  ...
  if lsof -i :6443 >/dev/null 2>&1; then
    warn "Port 6443 is in use. Checking for existing processes..."
    lsof -i :6443
    log "Existing cluster detected. Resetting kubeadm first..."
    ...
    # Clean up remaining mounts and directories
    umount -l /var/lib/kubelet/pods/*/volumes/kubernetes.io~projected/* 2>/dev/null || true
    rm -rf /var/lib/kubelet /etc/kubernetes /var/lib/etcd
    
    sleep 3
  fi
  
  # Ensure containerd is properly running
  log "Ensuring containerd is running properly..."
  ...
  if ! crictl info >/dev/null 2>&1; then
    warn "containerd not responding properly, restarting..."
    systemctl restart containerd
    sleep 10
  fi
  ...
  # Also set up for the user who ran sudo (if different from root)
  if [[ -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" ]]; then
    USER_HOME=$(getent passwd "$SUDO_USER" | cut -d: -f6)
    sudo -u "$SUDO_USER" mkdir -p "$USER_HOME/.kube"
    cp /etc/kubernetes/admin.conf "$USER_HOME/.kube/config"
    chown "$SUDO_USER:$SUDO_USER" "$USER_HOME/.kube/config"
    log "kubectl config also set up for user: $SUDO_USER"
  fi
  ...
  log "Control plane initialization complete!"
  log "To join worker nodes, run the kubeadm join command printed above."
...
To initialize as control plane, run:
  sudo bash install_kubernetes_kubeadm_ubuntu.sh --control-plane
